 ...
 *
 * Red Fort exception:
 * The Red Fort is a fortified Mughal palace complex, not a pyramidal temple
 * complex. Its secondary structures use low flat roofs with parapets. This is
 * applied by site slug so the existing archetype behaviour of all other sites
 * remains unchanged.
 */
...
export function Roofs({
  ...
  /*
   * The Red Fort should never inherit a pyramidal secondary-building roof.
   *
   * This is deliberately scoped to the site slug instead of changing the
   * global archetype rules, so Taj Mahal, Khajuraho, Konark, Hampi, etc.
   * continue using their existing roof systems.
   */
  const isRedFort = world.site.slug === 'red-fort'
  ...
  return (
    <group>
      {world.spaces.map((s) => {
        ...
        /*
         * Red Fort palace buildings are kept flat-roofed even if a stale or
         * incorrectly inherited archetype reaches this renderer.
         */
        const roof = isRedFort ? 'FLAT' : spec.roof
          ...
          <Fragment key={`r${s.space.id}`}>
            ...
            {isCore && spec.crown === 'ONION_DOME' && (
              <mesh
                ...
                geometry={sphere(
                  Math.min(s.rect.w, s.rect.d) * 0.46,
                  24,
                  14,
                  Math.PI * 2,
                  Math.PI / 2,
            ...
            /*
             * Pyramid roofs remain available for the monuments that actually
             * use them. Red Fort is excluded through the effective roof value
             * above.
             */
            {!isCore && roof === 'PYRAMID' && (
              <TieredRoof
                ...
                y={top + 0.84}
                tiers={7}
                tierH={Math.max(0.6, s.wallH * 0.11)}
                taper={0.845}
                material={stone}
                dark={dark}
            ...
            {!isCore && roof === 'TIERED' && (
              <TieredRoof
                ...
                y={top + 0.84}
                tiers={5}
                tierH={Math.max(0.9, s.wallH * 0.15)}
                taper={0.87}
                material={stone}
                dark={dark}
                shrine
            ...
            {!isCore && (roof === 'FLAT' || roof === 'DOMED_FLAT') && (
              <>
                ...
                {/*
                 * Red Fort palace roofs get restrained corner chhatris only
                 * on sufficiently substantial buildings. This prevents every
                 * tiny room from becoming a miniature temple.
                 */}
                {isRedFort &&
                  Math.min(w, d) >= 7 &&
                  [[-1, -1], [1, -1], [-1, 1], [1, 1]].map(
